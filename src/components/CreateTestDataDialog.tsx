...
export function CreateTestDataDialog({ children, onTestDataCreated }: CreateTestDataDialogProps) {
  ...
  // Scope
  const [visibility, setVisibility] = useState<'manual' | 'automated' | 'platform'>('manual');
  ...
  const simulateApiCall = async () => {
    ...
    return {
      ...
      customerName: customerType === 'Company' ? `Company ${customerId.slice(-3)}` : `User ${customerId.slice(-3)}`
    };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!customerType || !accountType || selectedClassifications.length === 0 || !ambiente || !dataOwner) {
          ...
          <Card className="border-blue-200 bg-blue-50">
            ...
            <CardContent>
              <p className="text-sm text-blue-700">
                Test data is created automatically through external API calls. 
                You only need to specify the desired characteristics and the system 
                will generate the corresponding accounts and entities.
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Select value={visibility} onValueChange={(value) => setVisibility(value as any)}>
                  ...
                  <SelectContent>
                    <SelectItem value="manual">Manual - Human use only from UI</SelectItem>
                    <SelectItem value="automated">Automated - Pre-pull by runners</SelectItem>
                    <SelectItem value="platform">Platform - Platform exclusive</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="text-sm text-gray-600 p-3 bg-gray-50 rounded-md">
                <strong>Note:</strong> The scope determines who can see and use this test data:
                <ul className="mt-2 space-y-1 list-disc list-inside">
                  <li><strong>Manual:</strong> Only human users from the interface</li>
                  <li><strong>Automated:</strong> CI/CD runners can access</li>
                  <li><strong>Platform:</strong> Only specific platforms can use it</li>
                </ul>
              </div>
            </CardContent>
          </Card>
