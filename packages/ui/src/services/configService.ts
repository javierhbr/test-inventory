...
import { configApi } from './apiClient';
...
/** Per-LOB configuration: each LOB gets its own runtimes and S3 config */
...
export interface SystemConfig {
  ...
  lobConfigurations: LobConfigurationSections;
  rolesPermissions: {
    title: string;
    description: string;
    cardStyle: {
  ...
  async loadSystemConfig(): Promise<SystemConfig> {
    if (!this.systemConfig) {
      try {
        ...
        this.systemConfig = {
          ...SYSTEM_CONFIG,
          lobConfigurations: data.lobConfig ?? SYSTEM_CONFIG.lobConfigurations,
          dsls: {
            ...SYSTEM_CONFIG.dsls,
